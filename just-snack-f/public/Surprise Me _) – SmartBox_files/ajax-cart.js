...
ShopifyAPI.addItemFromForm = function(form, callback, errorCallback) {
  ...
  params = {
    ...
    beforeSend: function(jqxhr, settings) {
      ...
      $('.AddToCartText', form).html('Adding..');
    },
    success: function(line_item) {
      if ((typeof callback) === 'function') {
        callback(line_item, form);
        $('.AddToCartText', form).html('Add To Cart');
...
var ajaxCart = (function(module, $) {
  ...
  buildCart = function (cart) {
    ...
    if (cart.item_count === 0) {
      $cartContainer
        .append('<p class="empty-cart">' + "Your cart is currently empty." + '</p>');
      cartCallback(cart);
      return;
    }
    ...
    $.each(cart.items, function(index, cartItem) {
      ...
      if(cartItem.properties_all != undefined){
        boldOptions = cartItem.properties_formatted;
      }
  ...
  cartCallback = function(cart) {
    ...
    //$('.cart-drop').addClass('show-cart-drop');
    
    $('.continue__shipping').click(function(){
      //$('.cart-drop').removeClass('show-cart-drop');
    });
  ...
  adjustCart = function () {
    ...
    $body.on('click', '.ajaxcart__remove-item', function() {
      if (isUpdating) {
        return;
      }
